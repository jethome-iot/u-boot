...
#define CONFIG_EXTRA_ENV_SETTINGS2 \
        ...
        "upgrade_check="\
            ...
            "if itest ${upgrade_step} == 3; then run storeargs; run init_hdmi; run update; fi;"\
            "\0"\
        "storeargs="\
            ...
            "setenv bootargs ${bootargs} androidboot.bootloader=${bootloader_version} androidboot.hardware=jethubj82 audit=${AuditEnabled};"\
            "run cmdline_keys;"\
            "\0"\
        "switch_bootmode="\
            "get_rebootmode;"\
        ...
        "storeboot="\
            ...
            "\0" \
         "update="\
            /*first usb burning, second usb_burn, third sdc_burn, fourth ext-sd autoscr/recovery, last udisk autoscr/recovery*/\
            "run usb_burning; "\
            "usb_burn usb_burn.ini; "\
        ...
        "cmdline_keys="\
            ...
            "if keyman init 0x1234; then "\
				...
				"else "\
					"setenv bootargs ${bootargs} androidboot.serialno=jethubj82${cpu_id};"\
					"setenv serial jethubj82{cpu_id}; setenv serial# jethubj82{cpu_id};"\
				"fi;"\
        ...
        "upgrade_key="\
            "saradc open 0; " \
            "if saradc get_in_range 0 100; then " \
                "msleep 50; " \
                "if saradc get_in_range 0 100; then echo update by key...; run update; fi; " \
            "fi;" \
		"\0"\
