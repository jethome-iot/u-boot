...
#define ENV_ARGS_RESERVE_DEFAULT \
	"lock", \
	"upgrade_step", \
	"bootloader_version"

#define ENV_ARGS_RESERVE_DISPLAY \
	"hdmimode",\
	"colorattribute",\
	"display_height",\
	"display_width",\
...
int checkhw(char * name)
{
	...
	switch (ddr_size) {
		case 0x80000000:
			strcpy(loc_name, "jethubj82_2g\0");
			break;
		default:
			printf("DDR size: 0x%x, multi-dtb not supported\n", ddr_size);
			strcpy(loc_name, "jethubj82_unsupport\0");
			break;
	}
	...
	return 0;
#else
	strcpy(name, "jethubj82_2g\0");
	env_set("aml_dt", "jethubj82_2g\0");
	return 0;
#endif
}
#endif
...
{
	// Keep in mind defenv_without.c::MaxReservNum
	ENV_ARGS_RESERVE_DEFAULT,
